...
export const Slide29DosDonts: React.FC<{ slide: SlideContent }> = ({ slide }) => {
  const { dos, donts } = slide.data;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col justify-center h-full py-2 px-2 sm:px-6">
      <div className="flex items-center gap-2 mb-2 animate-slide-up">
        ...
        <span className="text-xs font-mono text-slate-400">SLIDE 29 / 31</span>
      </div>
      ...
      <p className="text-xs sm:text-sm text-slate-400 mb-4 animate-slide-up stagger-2">
        Guiding execution principles to maintain professional integrity and stellar brand value:
      </p>

      {/* Two Columns Comparison */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {/* DO Column */}
        <div className="p-4 sm:p-5 rounded-3xl bg-slate-900/90 border-2 border-emerald-500/40 shadow-xl shadow-emerald-950/20 animate-slide-up stagger-2">
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-emerald-500/20">
            <h3 className="text-base sm:text-lg font-bold text-emerald-300 flex items-center gap-2 font-display">
              <CheckCircle2 className="w-5 h-5 text-emerald-400" /> WHAT TO DO (✅)
            </h3>
          ...
          <div className="space-y-2 overflow-y-auto max-h-[48vh] pr-1">
            {dos.map((item: any, idx: number) => (
              <div key={idx} className="p-2.5 rounded-xl bg-slate-800/60 border border-slate-750 flex flex-col">
                ...
                <span className="text-[11px] text-slate-300 mt-0.5 ml-5 leading-tight">{item.detail}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* DON'T Column */}
        <div className="p-4 sm:p-5 rounded-3xl bg-slate-900/90 border-2 border-rose-500/40 shadow-xl shadow-rose-950/20 animate-slide-up stagger-3">
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-rose-500/20">
            <h3 className="text-base sm:text-lg font-bold text-rose-300 flex items-center gap-2 font-display">
              <XCircle className="w-5 h-5 text-rose-400" /> WHAT TO AVOID (❌)
            </h3>
